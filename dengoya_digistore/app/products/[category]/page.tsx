      ...
      <main className="container mx-auto px-6 py-12 flex-1">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 w-full md:w-auto">
            <Link href="/products" className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg font-medium text-sm whitespace-nowrap transition-colors">Tout</Link>

            <Link
              href="/products/formation"
              className={`px-4 py-2 rounded-lg font-medium text-sm whitespace-nowrap transition-colors ${category === 'formation' ? 'bg-primary text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'}`}
            >
              Formation
            </Link>
            <Link
              href="/products/logiciels"
              className={`px-4 py-2 rounded-lg font-medium text-sm whitespace-nowrap transition-colors ${category === 'logiciels' ? 'bg-primary text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'}`}
            >
              Logiciels
            </Link>
            <Link
              href="/products/outils"
              className={`px-4 py-2 rounded-lg font-medium text-sm whitespace-nowrap transition-colors ${category === 'outils' ? 'bg-primary text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'}`}
            >
              Outils
            </Link>
          </div>
